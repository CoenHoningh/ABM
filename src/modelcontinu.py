...
class RoadSim(Model):
    def __init__(self, lanes=2, length=500, spawn_chance=0.3):
        ...
        self.grid = ContinuousSpace(self.length, self.lanes * 10, torus=False)
    ...
    def step(self):
        ...
        self.carplot.set_offsets([(car[0], car[1])
                                        for car in self.grid._agent_points])
    ...
    def visualise(self):
        ...
        self.plot.hlines(range(self.lanes + 1), 0, self.length)

        # print(list(list(zip(*self.cars))[0]))
        self.carplot = self.plot.scatter([car[0] for car in self.grid._agent_points],
                                [car[1] for car in self.grid._agent_points], s=100, marker='s')
