...
class Grid:
    ...
    def move_to_empty(self, agent):
        ...
        new_pos = agent.random.sample(self.empties, 1)
        self._place_agent(new_pos)
        agent.pos = new_pos
        self._remove_agent(pos)
    ...
    def find_empty(self):
        ...
        if self.exists_empty_cells():
            pos = random.sample(self.empties, 1)
            return pos
        else:
            return None
...
class SingleGrid(Grid):
    ...
    def position_agent(self, agent, x="random", y="random"):
        ...
        if x == "random" or y == "random":
            ...
            coords = agent.random.sample(self.empties, 1)
        else:
            coords = (x, y)
        agent.pos = coords
        self._place_agent(coords)

    def _place_agent(self, pos):
        if self.is_cell_empty(pos):
            super()._place_agent(pos)
        else:
